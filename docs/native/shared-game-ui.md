...
以上为 macOS 上的真实游戏和 GPU 证据；组字是 SDL 事件注入，未验证 OS 候选窗。
提示条已接入共享渲染，但本轮没有触发剧情退款来单独验收提示条。

## 尚未完成的跨平台工作

这次切换完成的是**默认游戏页面**。`graphics.cpp` 的 Metal surface/readback/present completion、
`native_dialogue_text.cpp` 的 CoreText 字形布局和渲染、原生 marker，以及桌面首次 ROM 选择器
仍有 macOS 实现；非 Apple 图形构建 gate 保留。Windows/Linux 游戏可玩性仍取决于这些 P2 工作。
没有以公共应用层 CI 或合成页面测试替代三平台游戏实测。
