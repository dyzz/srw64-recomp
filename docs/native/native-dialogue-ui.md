...
- 翻页位置用动态规划选：页数最少，其次少切在句子中间、少切在逗号处、少留一两个字的末行（标点和引号不算字）；这几项都相同时，前面的页排满。原版翻页点只在日文里算句末：译文自己标出句末，没有标点的原翻页处是一句话跨页接着说。中文行末的句读标点能因此多放一个字时压成半宽。
  - 这三处细则是 2026-09-24 按补过页末标点的台词定的。抽 1,500 条多页记录排版，页数不变：中文切在句子中间的页 116 → 31、第一页只有一行的 156 → 55；英文分别是 229 → 80、164 → 77。
