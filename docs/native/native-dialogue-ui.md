...
- **实机验证（2026-09-24，`tools/recomp/debug/check_dialogue.py`）：** 新游戏选默认主角与姓名，从路线开场读 18 条对白（15 条中文、3 条英文，其中 11 条有原版翻页点，最多 3 个），25 项检查全部通过。
  - 每条的后台确认次数都等于 `<STOP>` 数，`<END>` 恰好一次；原版从没跑到宿主页前面。
  - 中英文各两次 I/K，当前页开头都不变。
  - 三次 F7 轮换中→英→日→中（或英→日→中→英），每次都从原版当前片段开头接着读，原版不前进。
  - 截图里名字行与正文不重叠，三行正文都在框内。
  - 看到的内容问题：机翻中文在原版翻页处常常没有标点，连排后读成一句（如“傻的是你游击队员在战斗中败北……”）；默认主角和搭档名仍是日文片假名。
