...
| 其他行 | 译文。一页通常写成一行，由程序按对白框宽度自动折行。手动换行会在游戏里强制另起一行，只在确实需要时使用 |
| `---` | 原版的翻页处（原版按一次 A 才出现的下一页）。个数必须和原文一样，游戏靠它和原版的推进对齐。游戏里一条对白整条连起来，按对白框的大小重新分页，不一定在这里翻页；逐字显示到这里会停约 0.3 秒，分页时也优先在这里断开 |
...
- 游戏内怎样排版（整条连排、每页几行、英文字号、翻页位置怎么选）见[对白排版](../design/dialogue-typesetting.md)。
