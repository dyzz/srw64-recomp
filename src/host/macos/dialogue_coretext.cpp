// macOS CPU text backend. Intentionally independent of RT64, Plume and Metal.
...
RasterizedFrame rasterize_frame(const Frame& frame,uint32_t width,uint32_t height) {
    ...
    json report={{"schema","srw64.native-dialogue-raster.v1"},{"font",localization::catalog().font},{"locale",localization::catalog().locale},
        {"renderer","Core Text + Core Graphics"},{"native_vi",frame.vi},
...
Layout typeset(const std::u16string& value,unsigned size,double width,double height) {
    if(!size || size>256 || !std::isfinite(width) || !std::isfinite(height) || width<=0 || height<=0)
        throw std::runtime_error("Invalid native text layout dimensions");
    ...
    // No page can have more lines than code units. Cap before the floating
    // point to integer conversion, including for very large finite heights.
    const size_t per_page=static_cast<size_t>(std::clamp(std::floor(height/(size*1.22)),
        1.0,double(std::max<size_t>(1,value.size()))));
    Page page;
