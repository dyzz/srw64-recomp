...
def prepare_battle_assets(root: Path, rom: bytes, output: Path) -> dict:
    ...
    for actor in range(spec['count']):
        ...
        portraits[str(actor)] = save(f'face-{image_id}-{palette_id}',
                                    decode_indexed(resource(image_id), resource(palette_id)),
                                    (image_id, palette_id))
