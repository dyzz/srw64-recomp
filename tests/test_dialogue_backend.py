...
class DialogueBackendBoundaryTests(unittest.TestCase):
    ...
    def test_cpu_backend_has_no_gpu_state_or_file_output(self):
        source = (ROOT / "src/host/macos/dialogue_coretext.cpp").read_text(encoding="utf-8")
    ...
    def test_compositor_does_not_typeset(self):
        source = (ROOT / "src/host/macos/dialogue_metal.cpp").read_text(encoding="utf-8")
        for name in ("CoreText/", "CoreGraphics/", "CTTypesetter", "CGContext", "CFString"):
            self.assertNotIn(name, source)
        self.assertIn("rasterize_frame(frame,width,height)", source)
        self.assertIn("raster.image.validate()", source)
