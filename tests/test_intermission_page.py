...
class UpgradePageSourceTests(unittest.TestCase):
    ...
    def test_save_hooks_are_renamed_and_replaced(self):
        generator = (ROOT / "tools/recomp/toolchain/generate_cpu.py").read_text()
        hooks = (ROOT / "src/host/game_hooks.cpp").read_text()
        for address, original, screen in (("801CEA30", "srw64_original_save_choice_open", 1), ("801CEABC", "srw64_original_save_choice_step", 1),
                                          ("801CECE8", "srw64_original_save_slots_open", 9), ("801CEEF8", "srw64_original_save_slots_step", 9)):
            self.assertIn(f'"load_0008F4B0_func_{address}": "{original}"', generator)
            body = hooks[hooks.index(f"void load_0008F4B0_func_{address}("):]
            body = body[:body.index("\n}\n")]
            self.assertIn(f"{original}(rdram, ctx)", body)
            self.assertIn(f"save_{'build' if original.endswith('_open') else 'step'}(rdram, ctx, {screen}", body)
        self.assertIn("if(original_screens())return false;", (ROOT / "src/host/save_page.cpp").read_text())
        if ROM.exists():
            rom = ROM.read_bytes()
            table = 0x801DC9D0 - 0x801C4500 + 0x8F4B0
            entries = [struct.unpack(">II", rom[table + n * 8:table + n * 8 + 8]) for n in range(23)]
            self.assertEqual([entries[n] for n in (1, 9)], [(0x801CEA30, 0x801CEABC), (0x801CECE8, 0x801CEEF8)])
