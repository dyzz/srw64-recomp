...
def main():
    ...
    for _ in range(40):
        ...
        subprocess.run([sys.executable, str(Path(__file__).resolve().parents[1] / "run" / "control_host.py"), str(output),
    ...
    subprocess.run([sys.executable, str(Path(__file__).resolve().parents[1] / "run" / "control_host.py"), str(output), "--quit"], check=True, stdout=subprocess.DEVNULL)
