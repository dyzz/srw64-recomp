...
SOURCE = ROOT / "build/hd-ai/dialogue-type/live-medium-4x"
PACK = ROOT / "build/hd-ai/dialogue-type/medium-pack/pack"
FONT = ROOT / "build/hd-ai/dialogue-polish/fonts/HarmonyOS_Sans_SC_Medium.ttf"
IDENTITY = {
...
def main() -> int:
    ...
    report = {"schema": "srw64.coretext-probe-acceptance.v1", "status": "passed",
              ...
              "source_sha256": {str(p.relative_to(ROOT)): digest(p) for p in [Path(__file__).resolve(),
                   ROOT/"tools/recomp/native-host/coretext_probe.cpp", ROOT/"tools/recomp/native-host/coretext_probe.hpp",
                   ROOT/"tools/recomp/native-host/graphics.cpp", ROOT/"tools/recomp/native-host/frame.cpp",
                   ROOT/"tools/recomp/native-host/replay_vi.hpp",
                   ROOT/"tools/recomp/native-host/CMakeLists.txt"]},
