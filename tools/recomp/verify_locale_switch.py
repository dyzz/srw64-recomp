...
def main() -> int:
    ...
    def pulse(button: str) -> None:
        subprocess.run([sys.executable, str(Path(__file__).with_name('control_host.py')),
    ...
    subprocess.run([sys.executable, str(Path(__file__).with_name('control_host.py')), str(output), '--quit'], check=True)
